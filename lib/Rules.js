...
Rules.prototype.runTest = function(expected, actual) {
  if (typeof expected === "object") {
    ...
    return this.comparators.equality.call(this, expected, actual);
  }
};

Rules.prototype.runOutcomes = function (outcomes, data) {
  var outcomeKey, outcomeArr;
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (keys[i].slice(-2) === "[]") {
      ...
      outcomeArr = path.get(data, outcomeKey) || [];
      outcomeArr.push(outcomes[keys[i]]);
      path.set(data, outcomeKey, outcomeArr);
    } else {
